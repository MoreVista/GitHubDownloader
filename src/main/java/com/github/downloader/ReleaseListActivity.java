...
public class ReleaseListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateBookmarkButton(owner, repo);
        btnBookmark.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                boolean added = bookmarkManager.toggle(owner, repo);
                updateBookmarkButton(owner, repo);
                android.widget.Toast.makeText(ReleaseListActivity.this,
                        added ? "★ ブックマークに追加しました" : "ブックマークから削除しました",
    ...
    private void updateBookmarkButton(String owner, String repo) {
        if (bookmarkManager.contains(owner, repo)) {
            btnBookmark.setText("★ ブックマーク済み（タップで解除）");
        } else {
            btnBookmark.setText("★ ブックマークに追加");
        }
    }
